...
SpirvEmitter::processReverseBitsIntrinsic(const CallExpr *callExpr,
  ...
  if (bitwidth == 32) {
    ...
    // Get the lower half bits.
    clang::spirv::SpirvUnaryOp *lowerUint;
    auto *constMaskLower = spvBuilder.getConstantInt(
        astContext.UnsignedLongLongTy, llvm::APInt(64, 0x00000000ffffffff));
    if (isVectorType(argType)) {
      std::vector<clang::spirv::SpirvConstant *> vecLower(count,
                                                          constMaskLower);
      auto *vectorMaskLower =
          spvBuilder.getConstantComposite(argType, vecLower);
      auto *lowerHalf = spvBuilder.createBinaryOp(
          spv::Op::OpBitwiseAnd, argType, loadInst, vectorMaskLower, srcLoc);
      // Cast to uint values.
      lowerUint = spvBuilder.createUnaryOp(
          spv::Op::OpUConvert,
          astContext.getExtVectorType(astContext.UnsignedIntTy, count),
          lowerHalf, srcLoc);
    } else {
      auto *lowerHalf = spvBuilder.createBinaryOp(
          spv::Op::OpBitwiseAnd, argType, loadInst, constMaskLower, srcLoc);
      // Cast to uint value.
      lowerUint = spvBuilder.createUnaryOp(
          spv::Op::OpUConvert,
          astContext.getExtVectorType(astContext.UnsignedIntTy, count),
